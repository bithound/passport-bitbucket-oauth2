...
function Strategy(options, verify) {
  ...
  this._userEmailURL = options.userProfileURL || 'https://api.bitbucket.org/2.0/user/emails';
 ...
 *   - `emails`           the user's Bitbucket emails
 *
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    self._oauth2.get(self._userEmailURL, accessToken, function (err, body, res) {
      if(!err) {
        //Add emails to json object
        json.emails = JSON.parse(body).values;
      }
